...
class productController {
  ...
  static updateProductView = async (req, res) => {
    ...
    let filter = { isDeleted: false, _id: mongoose.Types.ObjectId(req.params.productId) }
  ...
  static addProduct = async (req, res, next) => {
    try {
      ...
      let matchProductName = await productModel.findOne({ name: req.body.name, isDeleted: false });

      if (matchProductName) {
        return send(res, statusCode.NOT_ACCEPTABLE, message.PRODUCT_ALREADY_EXISTS, null)
      }

      if (!req.files || Object.keys(req.files).length === 0) {
        return send(res, statusCode.REQUIRED_CODE, message.PRODUCT_IMAGE_REQUIRED, null)
      }
      ...
      return send(res, statusCode.SUCCESSFUL, message.SUCCESSFULLY_ADDED, { "_id": productData._id })
    } catch (error) {
      next(error)
    }
  };
  ...
  static productList = async (req, res, next) => {
    try {
      ...
      var sortType = !req.query.sortType ? 1 : req.query.sortType.toLowerCase() == "desc" ? -1 : 1;
      ...
      return res.send(
        services.responseData(statusCode.SUCCESSFUL, message.SUCCESSFUL, productData, totalRecord)
      );
    } catch (error) {
      next(error)
    }
  ...
  static productListById = async (req, res, next) => {
    try {
      ...
      const checkProductId = await productModel.findOne({ "_id": req.params.productId, isDeleted: false })
      if (!checkProductId) {
        return send(res, statusCode.NOT_ACCEPTABLE, message.PRODUCT_NOT_FOUND, null)
      }
      ...
      return send(res, statusCode.SUCCESSFUL, message.SUCCESSFUL, productData[0])
    } catch (error) {
      next(error)
    }
  };
  ...
  static updateProduct = async (req, res, next) => {
    try {
      ...
      const checkName = await productModel.findOne({ "_id": { $ne: req.params.productId }, "name": req.body.name })
      if (checkName) {
        return send(res, statusCode.NOT_ACCEPTABLE, message.PRODUCT_ALREADY_EXISTS, null)
      }
      ...
      if (req.files) {
        let saveImage = services.imageUploader(req.files.image, req.params.productId);
        const imagePath = await imageModel.insertMany(saveImage);
      }

      return send(res, statusCode.SUCCESSFUL, message.SUCCESSFULLY_UPDATED, { "_id": productData._id })
    } catch (error) {
      next(error)
    }
  };
  ...
  static deleteImage = async (req, res, next) => {
    try {
      const checkImageId = await imageModel.findOne({ "_id": req.params.imageId, isDeleted: false })
      if (!checkImageId) {
        return send(res, statusCode.NOT_ACCEPTABLE, message.IMAGE_NOT_FOUND, null)
      }
      ...
      const imageData = await imageModel.deleteOne({ "_id": mongoose.Types.ObjectId(req.params.imageId) })
      ...
      return send(res, statusCode.SUCCESSFUL, message.IMAGE_DELETED_SUCCESSFULLY, null)
    } catch (error) {
      next(error)
    }
  }
