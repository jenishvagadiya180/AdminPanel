...
class categoryController {
  ...
  static addCategory = async (req, res, next) => {
    try {
      ...
      let matchName = await categoryModel.findOne({
        name: req.body.name,
        isDeleted: false,
      });
  ...
  static categoryList = async (req, res, next) => {
    try {
      ...
      var sortType =
        req.query.sortType == null || !req.query.sortType
          ? 1
          : req.query.sortType.toLowerCase() == "desc"
          ? -1
          : 1;
      var sortField =
        req.query.sortField == null || !req.query.sortField
          ? "name"
          : req.query.sortField;
  ...
  static categoryListById = async (req, res, next) => {
    try {
      ...
      const categoryData = await categoryModel.find(
        { isDeleted: false, _id: req.params.categoryId },
        { name: 1 }
      );
